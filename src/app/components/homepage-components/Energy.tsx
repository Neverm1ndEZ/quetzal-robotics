...
const Stat: React.FC<StatProps> = ({ value, label }) => (
	<div className="group cursor-pointer">
		<h1 className="text-3xl lg:text-4xl font-light w-full transition-all duration-300 ease-out group-hover:-translate-y-2 group-hover:text-red-700">
			{value}
		</h1>
		<p className="font-light uppercase text-xs pt-2 transition-colors duration-300 group-hover:text-red-600">
			{label}
		</p>
	</div>
);
...
const Energy: React.FC = () => {
	return (
		<div className="mt-16 px-4 lg:px-0">
			...
			<div className="rounded-xl overflow-hidden bg-gradient-to-br from-red-100 via-red-50 to-white">
				<div className="flex flex-col lg:flex-row justify-between items-start py-12 px-8 gap-y-10">
					<div className="flex flex-col items-start w-full lg:w-1/2 space-y-6">
						<h2 className="text-4xl lg:text-5xl font-bold text-red-800 leading-tight">
							Clean Energy for a <br />
							Sustainable Future
						</h2>
						<p className="text-base lg:text-lg max-w-md text-red-700">
					...
					<div className="flex flex-col items-start w-full lg:w-1/2 space-y-10">
						...
						<Link href={`#contact`}>
							<button className="group relative overflow-hidden rounded-full bg-red-600 px-8 py-4 transition-all duration-300 hover:bg-red-700">
								<span className="relative z-10 flex items-center justify-center text-white transition-all duration-300 group-hover:text-red-100">
									Get in Touch
